...
export default function AstroFinaleSection() {
  const tCommon = useTranslations('Finale.common');
  const tC = useTranslations('Common');
  const t = useTranslations('Finale.astro');
  const tBody = useTranslations('FinaleBody.astro');

  const scrollTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  const shareText = encodeURIComponent(
    'Copernicus got the credit. Galileo got the telescope. Messier got the catalog. But al-Sufi found Andromeda (800 yrs earlier), al-Tusi invented the Tusi Couple (296 yrs earlier), and Ibn al-Shatir\'s lunar model is mathematically identical to Copernicus\'s. An interactive SDAM investigation.'
  );
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{
          background: 'radial-gradient(circle at 50% 40%, rgba(212,175,55,0.16) 0%, transparent 60%)',
        }}
      />
      ...
      <div className="max-w-4xl w-full relative z-10 text-center">
        ...
        >
          ...
          <p className="text-cream-dim text-sm mb-4">
            Algebra · Medicine · Trigonometry · The hospital · Navigation ·
            Cartography…
          </p>
        ...
        >
          ...
          <p className="font-mono text-[10px] uppercase tracking-widest text-cream-dim/60 mt-2">
            Seven pillars · CES 17/21 · Very Strong Case · Astronomy + The Library Theft
          </p>
        </motion.div>
      </div>
    </section>
