...
export default function MethodFinaleSection() {
  const tCommon = useTranslations('Finale.common');
  const tC = useTranslations('Common');
  const t = useTranslations('Finale.method');
  const tBody = useTranslations('FinaleBody.method');

  const scrollTop = () =>
    window.scrollTo({ top: 0, behavior: 'smooth' });

  const shareText = encodeURIComponent(
    'Francis Bacon got the credit. Descartes got the name. Popper got the criterion. Ibn al-Haytham got the method — 600 years earlier. An interactive SDAM investigation into who really founded the scientific method.'
  );
      ...
      <div className="max-w-4xl w-full relative z-10 text-center">
        ...
        >
          ...
          <p className="text-cream-dim text-sm mb-4">
            Algebra · Calculus · Trigonometry · Medicine · Astronomy · The
            heliocentric model…
          </p>
        ...
        >
          ...
          <p className="font-mono text-[10px] uppercase tracking-widest text-cream-dim/60 mt-2">
            Seven pillars · CES 17/21 · Very Strong Case · The Scientific Method
          </p>
        </motion.div>
      </div>
    </section>
