...
export default function AlgVerdictSection() {
    ...
    <section id="verdict" ref={ref} className="relative min-h-screen flex flex-col items-center justify-center px-6 py-24 overflow-hidden">
      ...
      <div className="max-w-4xl w-full relative z-20 text-center">
        ...
        <AnimatePresence>{showVerdict && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className="mt-12 max-w-2xl mx-auto space-y-4">
          <p className="text-cream text-base sm:text-lg leading-relaxed">The historical record is unambiguous. The word <span className="text-gradient-gold font-semibold">algorithm</span> is a Latinized form of <span className="text-gradient-gold font-semibold">al-Khwārizmī</span>. The word <span className="text-gradient-gold font-semibold">algebra</span> is the title of his book. These are not analogies or similarities — they ARE his name and his book title. Every algorithm ever written, every line of code ever compiled, every computation ever performed carries his name.</p>
          <p className="text-cream-dim text-sm sm:text-base leading-relaxed">This is the only SDAM case where Pillar VI (Etymology) is rated CONCLUSIVE — the word itself is the proof. Fibonacci explicitly credited &ldquo;the Arabs&rdquo; in 1202. Turing formalized the concept in 1936 without mentioning the etymology. The name survived as a word. The person was erased. <span className="text-gold">The name is everywhere. He is nowhere.</span></p>
