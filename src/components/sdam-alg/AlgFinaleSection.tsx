...
export default function AlgFinaleSection() {
  const tCommon = useTranslations('Finale.common');
  const tC = useTranslations('Common');
  const t = useTranslations('Finale.alg');
  const tBody = useTranslations('FinaleBody.alg');

  const scrollTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });
  const shareText = encodeURIComponent('Fibonacci got the credit. Cardano got the cubic. Turing got the formalization. But the word "algorithm" IS al-Khwārizmī\'s name. The word "algebra" IS his book title. Every algorithm ever written carries his name. An interactive SDAM investigation.');
  const openShare = (platform: 'twitter' | 'facebook' | 'linkedin') => { const url = encodeURIComponent(window.location.href); const targets = { twitter: `https://twitter.com/intent/tweet?text=${shareText}&url=${url}`, facebook: `https://www.facebook.com/sharer/sharer.php?u=${url}`, linkedin: `https://www.linkedin.com/sharing/share-offsite/?url=${url}` }; window.open(targets[platform], '_blank', 'noopener,noreferrer,width=600,height=540'); };
  const shareButtons = [{ label: 'X / Twitter', platform: 'twitter' as const }, { label: 'Facebook', platform: 'facebook' as const }, { label: 'LinkedIn', platform: 'linkedin' as const }];
  return (
    <section id="finale" className="relative min-h-screen flex flex-col items-center justify-center px-6 py-24 overflow-hidden">
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 1.5 }} className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(circle at 50% 40%, rgba(212,175,55,0.18) 0%, transparent 60%)' }} />
      <div className="max-w-4xl w-full relative z-10 text-center">
        ...
        <motion.blockquote initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.8, delay: 0.3 }} className="glass-card-gold rounded-2xl p-8 sm:p-10 mb-10 relative">
          ...
          <p className="font-display text-base sm:text-xl md:text-2xl text-cream leading-relaxed italic">The systematic method of algebra, the quadratic formula, the Hindu-Arabic numeral system, and the concept of an algorithmic procedure were <span className="text-gradient-gold">developed by Muḥammad ibn Mūsā al-Khwārizmī in Baghdad, c. 820 CE</span> — 382 years before Fibonacci, 771 years before Vieta, and 1,116 years before Turing. The word <span className="text-gradient-gold">&ldquo;algorithm&rdquo;</span> is a Latinized form of his name (al-Khwārizmī → Algoritmi → algorismus → algorithm). The word <span className="text-gradient-gold">&ldquo;algebra&rdquo;</span> is the title of his book (<em>al-Jabr</em>). Fibonacci explicitly credited &ldquo;the Indians and the Arabs&rdquo; in 1202. The European tradition that followed credited Fibonacci. Turing formalized the concept in 1936 without mentioning the etymology. The name survived as a word. The person was erased.</p>
        </motion.blockquote>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.7, delay: 0.4 }} className="grid sm:grid-cols-3 gap-4 mb-12">
          ...
          ].map((p) => (<div key={p.num} className="glass-card rounded-xl p-5 text-left"><p className="font-mono text-xs text-gold mb-1">{p.num}</p><p className="font-display text-base text-cream mb-1">{p.t}</p><p className="text-cream-dim text-xs leading-relaxed">{p.d}</p></div>))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.7, delay: 0.6 }} className="glass-card rounded-2xl p-6 sm:p-8 max-w-2xl mx-auto mb-10">
          ...
          <p className="text-cream-dim text-sm mb-4">Trigonometry · Cartography · Architecture · Cryptography · Music theory · Agriculture…</p>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.8, delay: 1 }} className="mt-16 pt-8 border-t border-gold/10">
          <p className="font-display text-sm text-cream-dim italic">{tBody('blockquote')}</p>
          <p className="font-mono text-[10px] uppercase tracking-widest text-cream-dim/60 mt-2">Seven pillars · CES 19/21 · Irrefutable · Algorithms &amp; Algebra</p>
        </motion.div>
      </div>
    </section>
  );
