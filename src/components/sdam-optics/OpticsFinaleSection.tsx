...
export default function OpticsFinaleSection() {
  const tCommon = useTranslations('Finale.common');
  const tC = useTranslations('Common');
  const t = useTranslations('Finale.optics');
  const tBody = useTranslations('FinaleBody.optics');

  const scrollTop = () =>
    window.scrollTo({ top: 0, behavior: 'smooth' });

  const shareText = encodeURIComponent(
    'Newton got the prism. Snell got the name. Daguerre got the patent. Ibn al-Haytham got the theorem — 683 years earlier. An interactive SDAM investigation into who really discovered optics.'
  );
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{
          background:
            'radial-gradient(circle at 50% 40%, rgba(212,175,55,0.14) 0%, transparent 60%)',
        }}
      />
      ...
      <div className="max-w-4xl w-full relative z-10 text-center">
        ...
        >
          ...
          <p className="text-cream-dim text-sm mb-4">
            Algebra · The experimental method · Calculus · Trigonometry ·
            Medicine · Astronomy…
          </p>
        ...
        >
          <p className="font-display text-sm text-cream-dim italic">
            ...
            <span className="text-gold">
              Scientific Discovery Attribution Methodology
            </span>
            .
          </p>
          <p className="font-mono text-[10px] uppercase tracking-widest text-cream-dim/60 mt-2">
            Seven pillars · CES 16.5/21 · Very Strong Case · Optics &amp; Photography
          </p>
        </motion.div>
      </div>
    </section>
