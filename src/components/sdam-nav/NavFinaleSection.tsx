...
export default function NavFinaleSection() {
  const tCommon = useTranslations('Finale.common');
  const tC = useTranslations('Common');
  const t = useTranslations('Finale.nav');
  const tBody = useTranslations('FinaleBody.nav');

  const scrollTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });
  const shareText = encodeURIComponent('Columbus got the credit. Vasco da Gama got the route. Henry got the school. But Mariam al-Asturlabiya built the instruments (548 yrs earlier), Ibn Majid wrote the manual (guided da Gama across the Indian Ocean), and al-Mahri refined the math. An interactive SDAM investigation into who really invented navigation.');
  const openShare = (platform: 'twitter' | 'facebook' | 'linkedin') => {
    const url = encodeURIComponent(window.location.href);
    const targets = { twitter: `https://twitter.com/intent/tweet?text=${shareText}&url=${url}`, facebook: `https://www.facebook.com/sharer/sharer.php?u=${url}`, linkedin: `https://www.linkedin.com/sharing/share-offsite/?url=${url}` };
