...
const MYTH_FIGURES = [
  { year: '1088 CE', name: 'University of Bologna', claim: 'Credited as "the first university in the world." The standard origin story of higher education.', note: 'Founded 229 years AFTER al-Qarawiyyin.' },
  { year: '1096 CE', name: 'University of Oxford', claim: 'Claimed as the oldest university in the English-speaking world.', note: 'Founded 237 years AFTER al-Qarawiyyin.' },
  { year: '1150 CE', name: 'University of Paris', claim: 'The model for medieval European universities. Theology-dominated curriculum.', note: 'Founded 291 years AFTER al-Qarawiyyin.' },
  { year: '1209 CE', name: 'University of Cambridge', claim: 'Founded by scholars fleeing Oxford. The second-oldest English university.', note: 'Founded 350 years AFTER al-Qarawiyyin.' },
];

export default function UniMythSection() {
  const tCommon = useTranslations('Myth.common');
  const t = useTranslations('Myth.uni');
            ...
            {MYTH_FIGURES.map((fig, i) => (
              <motion.div key={fig.name} initial={{ opacity: 0, y: 50, x: i % 2 === 0 ? -30 : 30 }} whileInView={{ opacity: 1, y: 0, x: 0 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.8, delay: 0.1 }} className={`relative pl-16 sm:pl-0 flex ${i % 2 === 0 ? 'sm:justify-start' : 'sm:justify-end'}`}>
                ...
                <div className={`glass-card rounded-2xl p-6 sm:p-8 max-w-md w-full ${i % 2 === 0 ? 'sm:mr-12' : 'sm:ml-12'}`}>
                  ...
                  <p className="text-cream-dim italic text-xs sm:text-sm border-l-2 border-burgundy/40 pl-3">{fig.note}</p>
                </div>
              </motion.div>
            ))}
          </div>
