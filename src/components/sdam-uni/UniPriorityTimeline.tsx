      ...
      <div className="max-w-6xl w-full">
        ...
        <div className="relative h-[500px] mb-12">
          ...
          <motion.div style={{ opacity: gap }} className="absolute top-1/2 -translate-y-1/2 h-12 -mt-6">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "0px 0px 15% 0px" }} transition={{ delay: 0.6 }} className="absolute -top-9 left-1/2 -translate-x-1/2 text-center" style={{ left: `${(positionFor(859) + positionFor(1088)) / 2}%` }}>
          ...
          {MILESTONES.map((m, i) => {
            ...
            return (
              <motion.div key={m.year} initial={{ opacity: 0, y: isTop ? -20 : 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "0px 0px 15% 0px" }} transition={{ duration: 0.6, delay: 0.2 + i * 0.1 }} className="absolute" style={{ left: `${x}%`, transform: 'translateX(-50%)', top: isTop ? '2%' : 'auto', bottom: isTop ? 'auto' : '2%' }}>
