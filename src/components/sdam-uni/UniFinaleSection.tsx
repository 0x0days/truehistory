...
export default function UniFinaleSection() {
  const tCommon = useTranslations('Finale.common');
  const tC = useTranslations('Common');
  const t = useTranslations('Finale.uni');
  const tBody = useTranslations('FinaleBody.uni');

  const scrollTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });
  const shareText = encodeURIComponent('Bologna got the credit. Oxford got the fame. But Fatima al-Fehri founded the world\'s oldest university in 859 CE — 229 years before Bologna. While Lubna managed 500,000 books, Europe was burning women for reading. An interactive SDAM investigation.');
    ...
    <section id="finale" className="relative min-h-screen flex flex-col items-center justify-center px-6 py-24 overflow-hidden">
      ...
      <div className="max-w-4xl w-full relative z-10 text-center">
        ...
        <motion.blockquote initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.8, delay: 0.3 }} className="glass-card-gold rounded-2xl p-8 sm:p-10 mb-10 relative">
          ...
          <p className="font-display text-base sm:text-xl md:text-2xl text-cream leading-relaxed italic">The world&apos;s oldest continuously operating university was founded by <span className="text-gradient-gold">Fatima al-Fehri in 859 CE</span> — 229 years before Bologna. The largest library of the medieval world was managed by <span className="text-gradient-gold">Lubna of Cordoba</span> — 500,000 volumes when Europe&apos;s largest had 600. <span className="text-gradient-gold">Dhayfa Khatun</span> financed universities and hospitals in Aleppo. In the same centuries, European women were denied souls, defined as &ldquo;defective males,&rdquo; and burned alive for practicing medicine. The Malleus Maleficarum (1486) prescribed legal woman-killing for 296 years. The university model — degree-granting, faculty-organized, curriculum-based — was inherited from the Arabic madrasa. The European &ldquo;Age of Universities&rdquo; was built on an Arabic foundation. The female founders were erased.</p>
        </motion.blockquote>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.7, delay: 0.6 }} className="glass-card rounded-2xl p-6 sm:p-8 max-w-2xl mx-auto mb-10">
          ...
          <p className="text-cream-dim text-sm mb-4">Algebra · Medicine · Trigonometry · Cartography · Architecture · Cryptography…</p>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.8, delay: 1 }} className="mt-16 pt-8 border-t border-gold/10">
          ...
          <p className="font-mono text-[10px] uppercase tracking-widest text-cream-dim/60 mt-2">Seven pillars · CES 15.5/21 · Very Strong Case · Universities &amp; Women in Science</p>
        </motion.div>
      </div>
    </section>
  );
