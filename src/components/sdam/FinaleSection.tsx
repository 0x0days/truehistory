...
export default function FinaleSection() {
  ...
  const t = useTranslations('Finale.gravity');
  const tBody = useTranslations('FinaleBody.gravity');

  const scrollTop = () =>
    window.scrollTo({ top: 0, behavior: 'smooth' });

  const shareText = encodeURIComponent(
    'Newton got the math. Al-Khāzini got the discovery — 566 years earlier. An interactive SDAM investigation.'
  );
    ...
    >
      ...
      <div className="max-w-4xl w-full relative z-10 text-center">
        ...
        >
          ...
          <p className="font-display text-lg sm:text-2xl md:text-3xl text-cream leading-relaxed italic">
            {tBody('blockquote')}
          </p>
        </motion.blockquote>
        ...
        >
          ...
          <p className="text-cream-dim text-sm mb-4">
            Algebra · The experimental method · Optics · Calculus · Trigonometry
        ...
        >
          ...
          <p className="font-mono text-[10px] uppercase tracking-widest text-cream-dim/60 mt-2">
            Seven pillars · CES 12/21 · Strong Case
          </p>
        </motion.div>
      </div>
    </section>
