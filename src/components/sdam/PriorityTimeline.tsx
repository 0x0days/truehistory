...
const MILESTONES = [
  { year: 1020, label: 'al-Bīrūnī', note: 'Attraction to Earth\'s center', side: 'top', color: '#f4d062' },
  { year: 1121, label: 'al-Khāzini', note: 'Weight varies with distance', side: 'bottom', color: '#f4d062' },
  { year: 1492, label: 'Columbus', note: 'Europe rediscovers the world', side: 'top', color: '#a39c87' },
  { year: 1543, label: 'Copernicus', note: 'Heliocentric model published', side: 'bottom', color: '#a39c87' },
  { year: 1638, label: 'Galileo', note: 'Kinematics of falling bodies', side: 'top', color: '#d4af37' },
  { year: 1687, label: 'Newton', note: 'Universal inverse-square law', side: 'bottom', color: '#d4af37' },
];
...
export default function PriorityTimeline() {
  ...
  const gapHighlight = useTransform(scrollYProgress, [0.35, 0.55], [0, 1]);
      ...
      <div className="max-w-6xl w-full">
        ...
        >
          ...
          <h2 className="font-display text-4xl sm:text-6xl md:text-7xl text-cream leading-tight">
            <span className="text-gradient-gold">566 years</span>
            <br />
            of forgotten physics.
          </h2>
          <p className="mt-6 text-cream-dim max-w-2xl mx-auto text-sm sm:text-base">
            ...
            <span className="text-gold">definitive</span>. Here is the full
            chronology.
          </p>
        </motion.div>

        {/* The timeline */}
        <div className="relative h-[440px] sm:h-[420px] mb-12">
          {/* Base line */}
          ...
          {/* The 566-year gap highlight */}
          <motion.div
            style={{ opacity: gapHighlight }}
            className="absolute top-1/2 -translate-y-1/2 h-12 -mt-6 rounded-full"
          >
            <div
              className="h-full rounded-full border-y-2 border-gold/40"
              style={{
                background:
                  'repeating-linear-gradient(45deg, rgba(212,175,55,0.08), rgba(212,175,55,0.08) 6px, transparent 6px, transparent 12px)',
                left: `${positionFor(1121)}%`,
                width: `${positionFor(1687) - positionFor(1121)}%`,
                position: 'absolute',
              }}
            />
            <motion.div
              ...
              style={{
                left: `${(positionFor(1121) + positionFor(1687)) / 2}%`,
              }}
            >
              <p className="font-display text-3xl sm:text-4xl text-gradient-gold whitespace-nowrap">
                566 years
              </p>
            ...
            return (
              ...
              >
                <div className="flex flex-col items-center">
                  {isTop && (
                    <>
                      <div className="glass-card rounded-xl px-3 py-2 mb-2 min-w-[110px] text-center">
                        <p
                  ...
                  {!isTop && (
                    <>
                      ...
                      />
                      <div className="glass-card rounded-xl px-3 py-2 mt-2 min-w-[110px] text-center">
                        <p
                          className="font-mono text-xs font-bold"
                          style={{ color: m.color }}
                        >
        ...
        >
          <div className="glass-card rounded-xl p-4 text-center">
            ...
            <p className="text-cream-dim text-xs mt-1">
              Paradigm shift: gravity is variable, not absolute
            </p>
          </div>
          <div className="glass-card rounded-xl p-4 text-center">
            <div className="w-3 h-3 rounded-full bg-cream-dim mx-auto mb-2" />
            <p className="font-display text-cream text-sm">European gap</p>
            <p className="text-cream-dim text-xs mt-1">
              Five+ centuries of forgotten transmission
            </p>
          </div>
          <div className="glass-card rounded-xl p-4 text-center">
            ...
            <p className="text-cream-dim text-xs mt-1">
              Newton universalizes the variable gravity concept
            </p>
          </div>
        </motion.div>
      </div>
