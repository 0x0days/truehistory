...
export default function MedFinaleSection() {
  const tCommon = useTranslations('Finale.common');
  const tC = useTranslations('Common');
  const t = useTranslations('Finale.med');
  const tBody = useTranslations('FinaleBody.med');

  const scrollTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });
  const shareText = encodeURIComponent('Vesalius got the anatomy. Harvey got the circulation. Pasteur got the germs. But the clinical trial was al-Rāzī\'s (900 CE), the Canon was Ibn Sīnā\'s (1025 CE — Europe\'s textbook for 600 years), and pulmonary circulation was Ibn al-Nafīs\'s (1242 CE — 386 years before Harvey). An interactive SDAM investigation.');
    ...
    <section id="finale" className="relative min-h-screen flex flex-col items-center justify-center px-6 py-24 overflow-hidden">
      ...
      <div className="max-w-4xl w-full relative z-10 text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.7, delay: 0.6 }} className="glass-card rounded-2xl p-6 sm:p-8 max-w-2xl mx-auto mb-10">
          ...
          <p className="text-cream-dim text-sm mb-4">Algebra · Trigonometry · Cartography · Architecture · Cryptography · Music theory…</p>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true, margin: "0px 0px 50% 0px" }} transition={{ duration: 0.8, delay: 1 }} className="mt-16 pt-8 border-t border-gold/10">
          ...
          <p className="font-mono text-[10px] uppercase tracking-widest text-cream-dim/60 mt-2">Seven pillars · CES 19/21 · Irrefutable · Medicine &amp; The Library Theft</p>
        </motion.div>
      </div>
    </section>
  );
